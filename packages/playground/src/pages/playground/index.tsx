...
type PlaygroundContextType = ContextValueType & {updateContext: UpdateContextType}

export const PlaygroundContext = React.createContext<PlaygroundContextType>(initialContext as any)

const Playground: React.FC = () => {
    const [contextValue, setContext] = useState<ContextValueType>(initialContext)

    const updateContext = useCallback((context: ContextValueType) => {
        ...
        if(context[LocalDBKey.WorkBench]!== undefined) {
            localDB.set(LocalDBKey.WorkBench, context[LocalDBKey.WorkBench])
        }
    }, [setContext])
