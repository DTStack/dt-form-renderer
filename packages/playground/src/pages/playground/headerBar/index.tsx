...
import { Checkbox, Select, Tooltip, Modal, message, Input, Form, Popconfirm } from 'antd';
import type { DefaultOptionType } from 'antd/lib/select';
import { SaveOutlined, FileAddOutlined, DeleteOutlined } from '@ant-design/icons';
import { useSelector } from 'react-redux';
import { RootState, useAppDispatch } from '@/store';
import { setWIP, createFile, updateFile, removeFile } from '@/store/reducers/workbenchSlice';
...
const HeaderBar: React.FC<HeaderBarProps> = (props) => {
    ...
    const { workbench: { workInProgress, files } } = useSelector<RootState, RootState>(state => state)
    ...
    const createFileRecord = () => {
        ...
        if(index === -1) {
            ...
            message.error('当前有未命名保存的文件记录 untitled，请先保存 untitled！')
        }
    }
        ...
        <>
            <div className="playground-header-bar">
                ...
                <Tooltip title="另存为文件记录">
                    <SaveOutlined onClick={() => { toggle(true) }}/>
                </Tooltip>
...
function SaveModal (props: SaveModalProps) {
    ...
    const dispatch = useAppDispatch();
    const { workbench: { workInProgress, files }, configure: { autoSave } } = useSelector<RootState, RootState>(state => state)
