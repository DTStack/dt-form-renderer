...
const FormRenderer: React.ForwardRefRenderFunction<FormInstance, FormRendererProps> = (props, ref) => {
    const {
        parsedJson,
        formServicePool,
        defaultExtraData,
    ...
    useEffect(() => {
        ...
        const willMountFields = currentFields.filter(f => !mountedFields.includes(f));
        /** 当某些字段挂载时触发 Immediate Action */
        if(willMountFields.length && mountedFieldsRef.current.length) {
            setTimeout(() => {
                willMountFields.forEach(mf => pubSubCenterRef.current.publishImmediateTriggerEvent(mf, form.getFieldsValue(), extraDataRef))
            })
        }
